...
hooks()->add_filter('get_relation_data', 'reminder_service_add', 10, 2);
                ...
                if (!empty($reminder['recurring_type']) && !empty($reminder['last_recurring_date'])) {
                    ...
                    if (strtotime(date('Y-m-d H:i:s')) >= strtotime($end_date)) {
                        ...
                        if (!empty($reminder['cycles']) && $reminder['recurring'] < $reminder['cycles']) {
                            $rel_data   = get_relation_data($reminder['rel_type'], $reminder['rel_id'], $reminder['services']);

                            if ($rel_data['type'] == "service") {
                                $data = service_relation_data($rel_data, $reminder);
...
function service_relation_data($rel_data, $reminder)
{
    $service_data = [];
    $amount = 0;
    foreach ($rel_data['extra'] as $service_id) {
...
function reminder_service_add($data, $related)
{

    if ($related['type'] == "service") {
        $related['extra'] = explode(",", $related['extra']);
        return $related;
    }
}
