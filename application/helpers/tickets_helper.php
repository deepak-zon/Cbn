...
function check_contact_data($id)
{
    if ($id) {
        $CI = &get_instance();
        $CI->db->select('tblclients.phonenumber, tblclients.account_number, tblclients.account_number, tblsystems.system_name, tblclients.address, tblclients.subscribed_service');
        $CI->db->from('tblcontacts');
        $CI->db->join('tblclients', 'tblcontacts.userid  = tblclients.userid ', 'inner');
