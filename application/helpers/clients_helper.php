...
function data_get_franchise($name)
{
    $CI = &get_instance();
    if ($CI->db->table_exists(db_prefix() . 'franchise')) {
        $CI->db->where('franchise_number', $name);
        $result = $CI->db->get(db_prefix() . 'franchise')->row_array();
        return $result;
    }
}

function data_get_system($name)
{
    $CI = &get_instance();
    if ($CI->db->table_exists(db_prefix() . 'systems')) {
        $result = $CI->db->where('system_name', $name)->get(db_prefix() . 'systems')->row_array();
        return $result;
    }
}
function data_franchise()
{
    $CI = &get_instance();
    if ($CI->db->table_exists(db_prefix() . 'franchise')) {
        $result = $CI->db->get(db_prefix() . 'franchise')->result_array();
        return $result;
    }
}

function data_franchise_get($id)
{
    $CI = &get_instance();
    return $CI->db->where('id', $id)->get(db_prefix() . 'franchise')->row_array();
}

function data_system_get($id)
{
    $CI = &get_instance();
    return $CI->db->where('id', $id)->get(db_prefix() . 'systems')->row_array();
}

function get_last_id()
{
    $CI = &get_instance();
    $CI->db->select_max('userid');
...
function data_system()
{
    $CI = &get_instance();
    if ($CI->db->table_exists(db_prefix() . 'systems')) {
        $result = $CI->db->get(db_prefix() . 'systems')->result_array();
        return $result;
    }
}
function get_customfield_value($action, $fieldid, $fieldto)
{
    $CI = &get_instance();
    if (preg_match("/\/(\d+)$/", $action, $matches)) {
        $res = $CI->db->where(array('relid' => $matches[1], 'fieldid' => $fieldid, 'fieldto' => $fieldto))->get(db_prefix() . 'customfieldsvalues')->row_array();
        $data = $res['value'];
    } else {
        $data = "";
    }
...
function customfield_value_get($action, $value)
{
    $CI = &get_instance();
    if (preg_match("/\/(\d+)$/", $action, $matches)) {
        $res = $CI->db->where('userid', $matches[1])->get(db_prefix() . 'clients')->row_array();
        $data = $res[$value];
    } else {
        $data = "";
    }
...
function system_value_get($action)
{
    $CI = &get_instance();
    if (preg_match("/\/(\d+)$/", $action, $matches)) {
        $res = $CI->db->where('userid', $matches[1])->get(db_prefix() . 'clients')->row_array();

        $CI->db->select('tblsystems.*');
        $CI->db->from('tblclients');
        $CI->db->join('tblsystems', 'tblclients.system = tblsystems.id', 'inner');
...
function update_data_client($table, $id, $data)
{
    $CI = &get_instance();
    $CI->db->set($data);
    $CI->db->where('userid', $id);
    $CI->db->update(db_prefix() . $table);
    return true;
}
