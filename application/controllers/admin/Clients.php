...
class Clients extends AdminController
{
    ...
    public function fencthise_data_set()
    {
        $franchise = $_GET;
        $franchise_id = $franchise['franchise_id'];
        $data = data_franchise_get($franchise_id);
        $system = data_system_get($data['system_id']);
        $system_option = '<option value="' . $system['id'] . '">' . $system['system_name'] . '</option>';
        if (preg_match("/\/(\d+)$/", $franchise['actionurl'], $matches)) {
            $account_number = $data['franchise_number'] . "-" . (1000000 + $matches[1]);
            update_data_client('clients', $matches[1], ['account_number' => $account_number, 'franchise' => $franchise_id]);
            echo json_encode(['status' => 'true', 'account' => $account_number, 'system_option' => $system_option]);
            //print_r($account_number);
        } else {
            $last_id = get_last_id();
            $updated_id = $last_id['userid'] + 1;
            $account_number = $data['franchise_number'] . "-" . (1000000 + $updated_id);
            echo json_encode(['status' => 'true', 'account' => $account_number, 'system_option' => $system_option]);
        }
    }
    public function system_data_set()
    {
        $system = $_GET;
        $system_id = $system['franchise_id'];
        if (preg_match("/\/(\d+)$/", $system['actionurl'], $matches)) {
            update_data_client('clients', $matches[1], ['system' => $system_id]);
        }
    }
