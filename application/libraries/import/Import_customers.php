...
class Import_customers extends App_import
{
    ...
    public function perform()
    {
        ...
        foreach ($this->getRows() as $rowNumber => $row) {
            ...
            for ($i = 0; $i < $totalDatabaseFields; $i++) {
                ...
                } elseif ($databaseFields[$i] == 'account_number') {
                    $duplicate = $this->isDuplicateAccount($row[$i]);
            ...
            if ($insert['franchise']) {
                $franchise = data_get_franchise($insert['franchise']);
                if ($franchise) {
                    $insert['franchise'] = $franchise['id'];
                } else {
                    if($insert['account_number']){
                        $franchise = data_get_franchise(substr($insert['account_number'], 0, 4));
                        if ($franchise) {
                            $insert['franchise'] = $franchise['id'];
                        } else {
                            $insert['franchise'] = "0";
            ...
            if ($insert['system']) {
                $system = data_get_system($insert['system']);
                if ($system) {
    ...
    private function isDuplicateAccount($account_number)
    {
        $pattern = '/^([0-9]){4}([-]){1}([0-1]){1}([0-9]){6}?$/';
        $check = preg_match($pattern, $account_number);
        if ($check == "1") {
            $res = total_rows(db_prefix() . 'clients', ['account_number' => $account_number]);
            if ($res == "1") {
                $data = "1";
            } else {
                $data = "0";
